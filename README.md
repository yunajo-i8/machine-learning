...
입력데이터와 정답이 함께 주어진 상태에서 모델이 학습하는 방법이다.

정답이 존재하는 문제에 적합하다.

(학습과정)

입력 데이터와 정답 제공 → 모델은 입력 데이터 통해 예측값 생성 → 예측값과 실제 정답 간의 오차 계산하여 비용 함수 구함 → 비용 함수 최소화되도록 최적화(오차 줄이기 위해 파라미터를 수정) 과정을 반복 → 새로운 입력 데이터에 대해 정확한 예측을 수행   
